...
async def test_knowledge_tree_relations(knowledge_tree):
    ...
    relations_after = await knowledge_tree.get_relations_async(
        parent_id, rel_type="child"
    )
...
async def test_learn_from_entry(knowledge_tree):
    ...
    # Learn from it
    new_id = await knowledge_tree.learn_from_entry_async(
        original_id, "New derived knowledge"
    )

    # Verify bidirectional relations
    orig_rels = await knowledge_tree.get_relations_async(
        original_id, rel_type="learned_from_this"
    )
    new_rels = await knowledge_tree.get_relations_async(
        new_id, rel_type="learned_this_from"
    )
...
async def test_nonexistent_operations(knowledge_tree):
    ...
    await knowledge_tree.add_tag_async(
        fake_id, "test_tag"
    )  # Should log warning but not raise
    await knowledge_tree.add_relation_async(
        fake_id, str(uuid.uuid4())
    )  # Should log warning but not raise
