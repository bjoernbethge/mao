    ...
    try:
        ...
        if client and hasattr(client, "async_shutdown"):
            ...
            try:
                ...
                logging.error(
                    f"MCPClient fixture: Error during async_shutdown: {e}",
                    exc_info=True,
                )
        elif client:
            logging.warning(
                "MCPClient fixture: MCPClient instance does not have async_shutdown method or it failed prior to finally."
            )
...
async def test_mcp_client_server_operations(mcp_client):
    ...
    if not all_servers:
        logging.warning(
            "No servers found in mcp_config.json, skipping server operations test"
        )
        return
    ...
    assert isinstance(
        active_servers, list
    ), "list_active_servers() should return a list"
    assert set(active_servers) == set(
        all_servers
    ), "All servers should initially be active"
    ...
    assert (
        test_server not in active_after_disable
    ), f"Server {test_server} should be inactive after disable_server()"
    ...
    assert (
        test_server in active_after_enable
    ), f"Server {test_server} should be active after enable_server()"
    assert set(active_after_enable) == set(
        all_servers
    ), "All servers should be active again"
...
async def test_mcp_client_get_tools(mcp_client):
    ...
    assert (
        all_servers
    ), "Dieser Test erfordert konfigurierte MCP-Server. Keine Server gefunden."

    active_servers = await asyncio.to_thread(mcp_client.list_active_servers)
    logging.info(f"Verfügbare Server: {active_servers}")
    assert (
        active_servers
    ), "Keine aktiven Server gefunden. Dieser Test erfordert funktionierende MCP-Server."
