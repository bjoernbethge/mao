    ...
    async def create_embeddings() -> Tuple[Embeddings, int]:
        ...
        try:
            if "text-embedding-3" in EMBED_MODEL:
                ...
                logging.info(f"Using OpenAI embeddings: {EMBED_MODEL} with dimensions {embed_dim}")
        ...
        try:
            ...
            logging.info(f"Using FastEmbed embeddings: {embed.model_name} with dimensions {embed_dim}")
            return embed, embed_dim
        except Exception as e:
...
class VectorStoreBase:
    ...
    def __init__(
        ...
        self.embed = None
        self.embed_dim = None
        self._embedding_provider = embedding_provider or EmbeddingProvider.create_embeddings
    ...
    async def async_init(self) -> 'VectorStoreBase':
        ...
        logging.info(f"{self.__class__.__name__}: Using embedding dim {self.embed_dim} "
    ...
    async def create(
        ...
        instance = cls(url, collection_name, recreate_on_dim_mismatch, embedding_provider)
    ...
    async def _ensure_collection_async(self) -> None:
        ...
        try:
            ...
            if exists:
                ...
                if col_dim is None:
                    logging.warning(f"Could not determine vector dimension for existing collection "
                    ...
                    if self.recreate_on_dim_mismatch:
                        logging.warning(f"{msg} RECREATING collection based on recreate_on_dim_mismatch=True.")
                        await self.async_client.recreate_collection(
                            collection_name=self.collection_name,
                            vectors_config={self.vector_name: VectorParams(size=self.embed_dim, distance=Distance.COSINE)}
                        )
                    else:
                        logging.error(f"{msg} NOT recreating automatically. Manual intervention may be required.")
            else:
                logging.info(f"Collection '{self.collection_name}' does not exist. Creating now with dim {self.embed_dim}.")
                await self.async_client.create_collection(
                    collection_name=self.collection_name,
                    vectors_config={self.vector_name: VectorParams(size=self.embed_dim, distance=Distance.COSINE)}
                )
            logging.info(f"Collection '{self.collection_name}' is ready with target dim {self.embed_dim}.")
        except Exception as e:
            logging.error(f"Error during collection setup for '{self.collection_name}': {e}")
            raise QdrantOperationError(f"Failed to ensure collection '{self.collection_name}': {e}") from e
    ...
    def _ensure_collection(self) -> None:
        ...
        if self.embed is None or self.embed_dim is None:
            ...
            self.embed, self.embed_dim = asyncio.run(EmbeddingProvider.create_embeddings())
    ...
    async def wait_for_index(self, timeout: float = 2.0, check_interval: float = 0.1) -> bool:
        """
    ...
    async def add_entry_async(self, text: str, tags: Optional[List[str]] = None) -> str:
        ...
        try:
            ...
            if len(vector) != self.embed_dim:
                logging.error(f"CRITICAL: Embedding vector for entry '{point_id}' has dim {len(vector)}, "
            ...
            logging.error(f"Failed to add entry to '{self.collection_name}' after retries: {e}")
    ...
    async def add_relation_async(self, from_id: str, to_id: str, rel_type: str = "related") -> bool:
    ...
    async def get_relations_async(self, point_id: str, rel_type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    # Synchronous wrapper
    def get_relations(self, point_id: str, rel_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """Synchronous wrapper for get_relations_async"""
        return asyncio.run(self.get_relations_async(point_id, rel_type))

    async def remove_relation_async(self, from_id: str, to_id: str, rel_type: Optional[str] = None) -> bool:
        ...
        rels = entry.get("relations", [])
        new_rels = [r for r in rels if not (r.get("id") == to_id and (rel_type is None or r.get("type") == rel_type))]
        ...
        if len(new_rels) < len(rels):
            try:
                ...
                logging.error(f"Failed to remove relation from {from_id} to {to_id}: {e}")
                return False
        return True  # No matching relation to remove

    # Synchronous wrapper
    def remove_relation(self, from_id: str, to_id: str, rel_type: Optional[str] = None) -> bool:
        """Synchronous wrapper for remove_relation_async"""
        return asyncio.run(self.remove_relation_async(from_id, to_id, rel_type))

    # Additional helper methods
    @retry(stop=DEFAULT_RETRY_STOP, wait=DEFAULT_RETRY_WAIT, retry=retry_if_exception_type(QDRANT_RETRY_EXCEPTION))
    async def add_entries_batch_async(self, texts: List[str], tags_list: Optional[List[List[str]]] = None) -> List[str]:
        """Add multiple entries in batch asynchronously"""
        if not texts:
            return []
            
        if tags_list and len(texts) != len(tags_list):
            raise ValueError(f"Number of texts ({len(texts)}) must match number of tag lists ({len(tags_list)})")
        ...
        try:
            ...
            logging.info(f"Added {len(texts)} entries in batch to {self.collection_name}")
            return point_ids
    ...
    # Synchronous wrapper
    def add_entries_batch(self, texts: List[str], tags_list: Optional[List[List[str]]] = None) -> List[str]:
        """Synchronous wrapper for add_entries_batch_async"""
        return asyncio.run(self.add_entries_batch_async(texts, tags_list))

    async def traverse_async(self, start_id: str, depth: int = 1, rel_types: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    # Synchronous wrapper
    def traverse(self, start_id: str, depth: int = 1, rel_types: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
class KnowledgeTree(VectorStoreBase):
    ...
    """
    def __init__(self, url: str = QDRANT_URL, collection_name: str = "knowledge_tree", recreate_on_dim_mismatch: bool = False):
        super().__init__(url=url, collection_name=collection_name, recreate_on_dim_mismatch=recreate_on_dim_mismatch)
    ...
    async def create(
        ...
        instance._embedding_provider = embedding_provider or EmbeddingProvider.create_embeddings
        return await instance.async_init()
    ...
    async def learn_from_experience_async(
        ...
        if related_knowledge_id:
            ...
            if knowledge_entry:
                await self.add_relation_async(exp_id, related_knowledge_id, rel_type="knowledge")
            else:
                logging.warning(f"Could not link experience {exp_id} to non-existent knowledge entry {related_knowledge_id}")
                
        return exp_id

    def learn_from_experience(self, text: str, related_knowledge_id: Optional[str] = None, tags: Optional[List[str]] = None) -> str:
        """Synchronous wrapper for learn_from_experience_async"""
        return asyncio.run(self.learn_from_experience_async(text, related_knowledge_id, tags))
    ...
    async def learn_from_entry_async(self, entry_id: str, new_text: str) -> str:
        ...
        if not original_entry:
            logging.warning(f"Cannot learn from non-existent entry {entry_id}. Creating new entry directly.")
...
class ExperienceTree(KnowledgeTree):
    ...
    """
    def __init__(self, url: str = QDRANT_URL, collection_name: str = "experience_tree", recreate_on_dim_mismatch: bool = False):
        super().__init__(url=url, collection_name=collection_name, recreate_on_dim_mismatch=recreate_on_dim_mismatch)
...
if __name__ == '__main__':
    ...
    async def run_tests():
        ...
        try:
            ...
            entry_id1 = await kt.add_entry_async("The sky is blue.", tags=["nature", "color"])
            entry_id2 = await kt.add_entry_async("The sun is bright.", tags=["nature", "light"])
            ...
            logging.error(f"An unexpected error occurred during tests: {e}", exc_info=True)
