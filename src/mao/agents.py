...
def get_default_callbacks() -> List[BaseCallbackHandler]:
    ...
    try:
        ...
        if os.environ.get("LANGSMITH_TRACING") == "true" and os.environ.get("LANGSMITH_API_KEY"):
            tracer = LangChainTracer(project_name=os.environ.get("LANGSMITH_PROJECT", "mcp-agents"))
    except ImportError:
        pass
    if tracer:
        return [tracer]
        ...
        class LoggingCallbackHandler(BaseCallbackHandler):
            def on_chain_end(self, outputs, **kwargs): logging.info(f"Chain finished: {outputs}")
            def on_chain_error(self, error, **kwargs): logging.error(f"Chain error: {error}")
            def on_llm_end(self, response, **kwargs): logging.info(f"LLM finished: {response}")
            def on_llm_error(self, error, **kwargs): logging.error(f"LLM error: {error}")
...
def _create_llm_client(
    ...
    **kwargs: Any
    ...
    if provider_lower == "openai":
        return ChatOpenAI(model=model_name, temperature=temperature, callbacks=actual_callbacks, streaming=stream, **llm_specific_kwargs)
    elif provider_lower == "anthropic":
        # Add Claude 3 tools beta header if needed
        if "claude-3" in model_name.lower() and "anthropic-beta" not in llm_specific_kwargs.get("default_headers", {}):
...
async def load_mcp_tools(mcp_client: Union[MCPClient, List[Dict[str, Any]], None]) -> List[Dict[str, Any]]:
    """
            ...
            try:
                ...
                for server_name in active_servers:
                    try:
                        ...
                        logging.debug(f"Loaded {len(server_tools)} tools from server '{server_name}'")
                        all_tools.extend(server_tools)
                    except Exception as e:
                        logging.error(f"Failed to load tools from server '{server_name}': {e}")
                
                logging.debug(f"Successfully loaded total of {len(all_tools)} tools from all active servers")
                return all_tools
            except Exception as e:
...
def _determine_tokenizer_for_trim(llm: BaseChatModel) -> Union[BaseChatModel, str]:
    ...
    if isinstance(llm, OllamaLLM):
        return llm.model
    elif not (hasattr(llm, 'get_num_tokens_from_messages') or hasattr(llm, 'get_num_tokens')):
        if hasattr(llm, 'model_name'): 
            return llm.model_name
        elif hasattr(llm, 'model'): 
            return llm.model
        else:
            logging.warning("Defaulting tokenizer for trim_messages to 'gpt-3.5-turbo'.")
...
class Agent:
    ...
    async def _retrieve_context(self, query: str, k: int = 3) -> str:
        ...
        if self.knowledge_tree:
            ...
            try:
                ...
                knowledge_hits = await asyncio.to_thread(self.knowledge_tree.search, query, k=k)
        ...
        if self.experience_tree:
            ...
            try:
                ...
                # Fallback für ältere Versionen oder nicht initialisierte Instanzen
                experience_hits = await asyncio.to_thread(self.experience_tree.search, query, k=k)
    ...
    async def _learn_experience(self, user_input: str, model_output: str, tags: Optional[List[str]] = None) -> None:
        ...
        if self.knowledge_tree and user_input:
            try:
                ...
                knowledge_hits = await asyncio.to_thread(self.knowledge_tree.search, user_input, k=1)
    ...
    async def _call_model_node(self, state: MessagesState, config: Optional[RunnableConfig] = None) -> Dict[str, List[BaseMessage]]:
        ...
        messages_for_llm: List[BaseMessage] = [SystemMessage(content=system_content)] + trimmed_messages_list
        ...
        try:
            ...
            if self.stream and self.token_callback:
                ...
                async for chunk in self.llm.astream(messages_for_llm, **llm_call_kwargs):
                    token = chunk.content if hasattr(chunk, "content") else str(chunk)
                    self.token_callback(token)
                    streamed_content += token
                    
                # After streaming, get the full response for state tracking
                invoked_response = await self.llm.ainvoke(messages_for_llm, **llm_call_kwargs)
            else:
                # Non-streaming mode
                invoked_response = await self.llm.ainvoke(messages_for_llm, **llm_call_kwargs)
    ...
    async def init_agent(self):
        ...
        try:
            ...
            elif hasattr(self.knowledge_tree, 'async_init') and self.knowledge_tree.embed is None:
                await self.knowledge_tree.async_init()
                
            if self.experience_tree is None:
                self.experience_tree = await ExperienceTree.create()
            elif hasattr(self.experience_tree, 'async_init') and self.experience_tree.embed is None:
            ...
            if self.use_react_agent and self.loaded_tools:
                ...
                logging.info(f"Agent '{self.name}' created as ReAct agent with {len(self.loaded_tools)} tools.")
            ...
            self.agent_runnable = workflow.compile(checkpointer=self.memory, name=self.name)
            logging.info(f"Agent '{self.name}' compiled successfully.")
            return self.agent_runnable
        except Exception as e:
            logging.error(f"Failed to initialize agent '{self.name}': {e}", exc_info=True)
            raise
...
class Supervisor:
    def __init__(
        ...
        **supervisor_kwargs: Any
        ...
        supervisor_kwargs_dict = {"add_handoff_back_messages": add_handoff_back_messages}
    ...
    async def _create_supervisor_workflow(self) -> None:
        ...
        try:
            ...
            self.app = workflow.compile(checkpointer=self.memory, name="global_supervisor")
        except Exception as e:
            logging.error(f"Supervisor workflow initialization failed: {e}", exc_info=True)
            raise
    ...
    async def invoke(self, messages: List[dict], thread_id: Optional[str] = None) -> Any:
        """Invokes the supervisor asynchronously with a list of messages."""
        if self.app is None: 
            raise RuntimeError("Supervisor must be initialized. Call init_supervisor() first.")
