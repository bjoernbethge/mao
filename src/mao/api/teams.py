...
async def create_team(
    ...
    if team.supervisor_id:
        ...
        if not supervisor:
            raise HTTPException(status_code=404, detail=f"Supervisor {team.supervisor_id} not found")
...
async def add_team_member(
    ...
    if "order_index" not in member_data_dict:
        ...
        highest_order = max([m.get("order_index", 0) or 0 for m in existing_members], default=0)
...
async def update_team_member(
    ...
    if not found:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} is not a member of team {team_id}")
    ...
    # This should not happen
    raise HTTPException(status_code=500, detail="Failed to retrieve updated team member")
...
async def create_supervisor(
    ...
    if not agent:
        raise HTTPException(status_code=404, detail=f"Agent {supervisor.agent_id} not found")
...
async def start_team(team_id: str, db: ConfigDB = Depends(get_config_db)):
    ...
    if not members:
        raise HTTPException(status_code=400, detail=f"Team {team_id} has no active members")
            ...
            if supervisor_config:
                ...
                if supervisor_agent_id not in active_agents:
                    ...
                    supervisor_agent_app = await create_agent(
                        ...
                        use_react_agent=supervisor_agent_config.get('use_react_agent', True),
                        max_tokens_trimmed=supervisor_agent_config.get('max_tokens_trimmed', 3000),
                        llm_specific_kwargs=supervisor_agent_config.get('llm_specific_kwargs')
                    )
                    active_agents[supervisor_agent_id] = {"agent": supervisor_agent_app, "config": supervisor_agent_config}
                ...
                supervisor_params = {
                    "add_handoff_back_messages": supervisor_config.get("add_handoff_back_messages", True),
                    "parallel_tool_calls": supervisor_config.get("parallel_tool_calls", True)
                }
                ...
                supervisor = Supervisor(
                    agents=agent_apps,
                    supervisor_provider=active_agents[supervisor_agent_id]["config"]["provider"],
                    supervisor_model_name=active_agents[supervisor_agent_id]["config"]["model_name"],
                    supervisor_system_prompt=supervisor_config.get("system_prompt") or "You are a supervisor that coordinates a team of specialized agents.",
...
async def stop_team(team_id: str):
    ...
    if team_id not in active_teams:
        raise HTTPException(status_code=404, detail=f"No running team with ID {team_id}")
...
async def chat_with_team(
    ...
    if agent_id not in active_agents:
        ...
        agent_config = await db.get_agent(agent_id)
        if not agent_config:
            raise HTTPException(status_code=404, detail=f"Supervisor agent {agent_id} not found")
        ...
        try:
            ...
            agent_app = await create_agent(
                ...
                model_name=agent_config['model_name'],
                agent_name=agent_config['name'],
                system_prompt=supervisor_config.get('system_prompt') or agent_config.get('system_prompt'),
                use_react_agent=agent_config.get('use_react_agent', True),
                max_tokens_trimmed=agent_config.get('max_tokens_trimmed', 3000),
                llm_specific_kwargs=agent_config.get('llm_specific_kwargs')
            )
            ...
            raise HTTPException(status_code=500, detail=f"Failed to start supervisor agent: {str(e)}")
    ...
    try:
            ...
            if member_agent_id not in active_agents:
                ...
                try:
                    ...
                    member_agent_app = await create_agent(
                        ...
                        use_react_agent=member_agent_config.get('use_react_agent', True),
                        max_tokens_trimmed=member_agent_config.get('max_tokens_trimmed', 3000),
                        llm_specific_kwargs=member_agent_config.get('llm_specific_kwargs')
                    )
                    
                    # Add agent to active agents
                    active_agents[member_agent_id] = {"agent": member_agent_app, "config": member_agent_config}
                except Exception as e:
                    logging.warning(f"Failed to start team member agent {member_agent_id}: {e}")
                    continue  # Skip this member
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to process team chat: {str(e)}"
        ) 