...
async def create_new_server(server: ServerCreate, db: ConfigDB = Depends(get_config_db)):
...
async def update_server_by_id(server_id: str, server: ServerUpdate, db: ConfigDB = Depends(get_config_db)):
    """Updates a server"""
...
async def create_new_tool(tool: ToolCreate, db: ConfigDB = Depends(get_config_db)):
    ...
    if tool.server_id:
        ...
        if not server:
            raise HTTPException(status_code=404, detail=f"Server {tool.server_id} not found")
...
async def list_all_tools(server_id: Optional[str] = None, db: ConfigDB = Depends(get_config_db)):
...
async def update_tool_by_id(tool_id: str, tool: ToolUpdate, db: ConfigDB = Depends(get_config_db)):
    """Updates a tool"""
