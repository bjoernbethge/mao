...
async def import_configuration(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Failed to import configuration"
            )
...
async def import_configuration_file(
    file: UploadFile = File(...), db: ConfigDB = Depends(get_config_db)
):
...
def reset_database(confirm: bool = False, db: ConfigDB = Depends(get_config_db)):
    ...
    if not confirm:
        raise HTTPException(
            status_code=400, detail="Confirmation required to reset database"
        )

    conn = db._connect()

    # Alle Tabellen leeren
    tables = ["agents", "tools", "mcp_servers", "agent_tools", "global_configs"]
